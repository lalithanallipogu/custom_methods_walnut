 ...
 * description: POST get member to ${url} with ${requestBody} using session $[averSessionId] and generated member $[memberId] with suffix ${suffix}
...
export async function triggerGetMemberWithSuffix(ctx: WalnutApiContext) {
  ...
  if (!memberId) {
    throw new Error(
      'memberId not found in runtime variables. Ensure the upload step that generates the member ID ran first.'
    );
  }

  // Build full member_id: e.g. "ICMEM-7723MFBV" + "-ult_trig_dupe_SCK6tt"
  const fullMemberId = memberId + (suffix || '');
  ctx.log('Getting member by generated ID with suffix: ' + fullMemberId);
  ...
  // Override member_id with the generated member ID + suffix
