...
/** @walnut_method
 * name: Replace Artifact Files With Dummy Data and Upload
 ...
 * actionType: custom_replace_artifacts_with_dummy_data
 * context: shared
 * needsLocator: false
 * category: File Transfer
 */
export async function replaceArtifactsWithDummyData(ctx: WalnutContext) {
  // ctx.args[0] = filePath1 artifact ref (from ${filePath1})
  // ctx.args[1] = filePath2 artifact ref (from ${filePath2})
  // ctx.args[2] = filePath3 artifact ref (from ${filePath3})
  // ctx.args[3] = filePath4 artifact ref (from ${filePath4})
  ...
  const fileRefs = [ctx.args[0], ctx.args[1], ctx.args[2], ctx.args[3]];
  ...
  // Process each of the 4 artifact references
  for (let i = 0; i < fileRefs.length; i++) {
    const fileRef = fileRefs[i];

    if (!fileRef) {
      ctx.log('File ref ' + (i + 1) + ' is empty, skipping...');
      continue;
    }

    ctx.log('Processing artifact ' + (i + 1) + ': ' + fileRef);
    ...
    if (!fs.existsSync(filePath)) {
      throw new Error('File ' + (i + 1) + ' not found at resolved path: ' + filePath);
    }
