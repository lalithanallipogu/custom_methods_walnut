 ...
 * description: Reuse generated member ID $[memberId], replace {{member_id}} in artifact ${claimArtifact} and upload to /TO_AVER/ via SFTP host ${sftphost} port ${sftpport} user ${sftpusername} password ${sftppassword} using batch $[batch]
...
export async function reuseMemberIdArtifactReplaceUpload1File(ctx: WalnutBaseContext) {
  ...
  // ctx.args[1] = claimArtifact (from ${claimArtifact}) — artifact ref for the claim file
  ...
  // Step 3: Build filename with shifted timestamp (2705 days forward)
