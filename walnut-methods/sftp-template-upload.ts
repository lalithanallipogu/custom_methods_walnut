 ...
 * description: Replace {{member_id}} with $[icmemId] in template ${localFilePath} with SFTP host ${sftphost} port ${sftpport} user ${sftpusername} password ${sftppassword} and upload to /TO_AVER/ with ${forwardDays} days forward
...
export async function sftpTemplateUpload(ctx: WalnutContext) {
  ...
  // ctx.args[6] = forward days (from ${forwardDays}) — number of days to shift date forward
  ...
  const forwardDays = parseInt(ctx.args[6], 10) || 2695;
