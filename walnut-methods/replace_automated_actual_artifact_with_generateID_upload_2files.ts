...
/** @walnut_method
 * name: Generate Member ID Replace and Upload 2 Artifact Files
 ...
 * actionType: custom_generate_member_replace_upload_2files_artifacts
 * context: shared
 * needsLocator: false
 * category: File Transfer
 */
export async function generateMemberReplaceUpload2FilesArtifacts(ctx: WalnutContext) {
  ...
  const fileRefs = [ctx.args[0], ctx.args[1]];
  ...
  if (uploadPairs.length === 0) {
    throw new Error('No valid file references provided. Nothing to upload.');
  }
  ...
  const tmpScript = path.join(tempDir, 'sftp_upload_member_2files_' + Date.now() + '.py');
