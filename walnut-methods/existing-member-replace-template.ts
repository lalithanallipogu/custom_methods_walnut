 ...
 * description: Artifact Use existing ICMEM ID from $[memberId], replace {{member_id}} in file ${filePath} and upload to /TO_AVER/ via SFTP host ${sftphost} port ${sftpport} user ${sftpusername} password ${sftppassword} and batch in $[batch]
...
export async function artifactExistingMemberReplaceUpload(ctx: WalnutContext) {
  ...
  // ctx.args[1] = filePath (from ${filePath})
  ...
  // Step 3: Replace all occurrences of {{member_id}} with the existing member ID
